...
-- Create storage bucket for videos (optional for file upload)
...
BEGIN
    ...
    IF NOT EXISTS (
        ...
        AND policyname = 'Users can upload videos for their accounts'
    ) THEN
        CREATE POLICY "Users can upload videos for their accounts" ON storage.objects
        FOR INSERT WITH CHECK (
            bucket_id = 'account-videos' AND auth.uid() IS NOT NULL
        );
    END IF;
    ...
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE schemaname = 'storage' AND tablename = 'objects' 
        AND policyname = 'Users can view video files'
    ) THEN
        CREATE POLICY "Users can view video files" ON storage.objects
        FOR SELECT USING (
            bucket_id = 'account-videos'
        );
    END IF;
    ...
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE schemaname = 'storage' AND tablename = 'objects' 
        AND policyname = 'Users can delete their video files'
    ) THEN
        CREATE POLICY "Users can delete their video files" ON storage.objects
        FOR DELETE USING (
            bucket_id = 'account-videos' AND auth.uid() IS NOT NULL
        );
    END IF;
